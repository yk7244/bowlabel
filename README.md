...
python3 app.py             # 일반 실행 (데이터 유지)
python3 app.py --reset     # DB 완전 초기화 — 확인 입력('reset') 필요, --yes로 생략
...
> **데이터 안전:** 서버는 auto-reload(디버그 리로더)를 켜지 않는다. 예전에는 export가
> `exports/`에 파일을 쓰면 리로더가 재시작하고, `--reset`으로 띄운 경우 재시작마다 DB가
> 초기화되어 데이터가 사라지는 문제가 있었다(v5.1에서 수정). `--reset`은 최초 실행에서
> 확인을 받은 경우에만 동작한다. **정기 백업 권장:** `cp data.db backups/data_$(date +%F).db`
...
3. **작업 배분** (아래 두 모드)
4. 라벨러들이 라벨링 (대시보드는 **해야 할 것/완료/전체** 필터가 있는 썸네일 그리드)
5. **갤러리에서 리뷰/관리** → **라벨러 + 완료/미완료 필터**로 특정 라벨러의 완료본만 골라 보고,
   프레임 클릭 시 그 라벨러 기준 리뷰로 열림. 이상한 프레임 삭제·담당자 재배정도 가능
...
- 줌: 휠 / `+` `−` / `F`(Fit)
- 저장: `S` · **저장 ▶ 다음** / `Enter` → 저장 후 **다음 미완료 프레임으로 자동 이동**
- **☰ 목록** 버튼 → 내 작업 대시보드로 복귀 (완료/미완료 필터로 이전 작업 재방문)
